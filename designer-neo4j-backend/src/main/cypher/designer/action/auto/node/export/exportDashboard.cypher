/*
@name designer.action.auto.node.export.exportDashboard
...
RETURN 'Export Dashboard' AS name,
       'Export' AS group,
       'download' AS icon,
       [] AS params,
       [] AS implicitParams,
...
/*
@name designer.action.auto.node.export.exportDashboard_do
...
MATCH (dashboard:_Identifiable {uuid: contextUuid})
WHERE dashboard:Dashboard

WITH custom.designer.export.exportDashboard(dashboard) AS cypher
