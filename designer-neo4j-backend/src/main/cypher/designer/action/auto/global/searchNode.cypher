...
MATCH (metagraph)-[:DEFINES]->(nodeDef:NodeDef)-[:HAS_TYPE]->(:Type)-[:HAS_PROPERTY]->(property:Property)
  WHERE any(word IN ["name", "caption", "title"] WHERE toLower(property.name) CONTAINS word)
...
CALL apoc.cypher.run("MATCH (node:" + nodeType + " {" + propertyToSearchBy + ": $propertyValue}) RETURN node LIMIT 100", {propertyValue: propertyValue}) YIELD value
